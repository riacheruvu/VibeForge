...
function script(name) {
  return path.join(ROOT, "skills", "vibecheckbench", "scripts", name);
}
...
function commandDraft(argv) {
  ...
  if (!text) {
    console.error("Please provide a preference, for example: vibecheckbench draft \"I want concise, honest answers\"");
    process.exitCode = 1;
    return;
  }
...
function commandDemo(argv) {
  ...
  runNode([script("create-fit-review.mjs"), preference, "--out", path.join("vibecheckbench-out", `demo-${kind}`), "--force"]);
}
...
function commandRun(argv) {
  ...
  runNode([
    ...
    limit,
    "--out",
    path.join("vibecheckbench-out", "answers.local-demo.json"),
    "--scored-out",
    path.join("vibecheckbench-out", "run-results.json"),
    "--chart-out",
...
function commandReport() {
  const runResults = readJsonIfExists(path.join("vibecheckbench-out", "run-results.json"));
  if (!runResults) {
    console.log("No run results found yet. Start with:");
    console.log('  vibecheckbench draft "I want concise, honest answers"');
    console.log("  vibecheckbench run");
    return;
  }
  if (runResults.status === "not_run") {
    console.log("A fit review exists, but no model/config comparison has been run yet.");
    console.log("Next: vibecheckbench run");
    return;
  }
  runNode([
...
function commandRecommend() {
  ...
  if (runResults && runResults.status !== "not_run") {
    runNode([
      ...
      path.join("vibecheckbench-out", "run-results.json"),
      "--out",
      path.join("vibecheckbench-out", "next-experiment.json"),
    ]);
    return;
  }

  const planned = readJsonIfExists(path.join("vibecheckbench-out", "next-experiment.json"));
