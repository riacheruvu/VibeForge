...
description: >
  ...
  routing) fits a person — not raw model IQ. Also known as VibeForge. Use for
  preference fit, fit reviews, setup comparison, case studies, Promptfoo
  preference suites, high-signal answers, pushback, anti-sycophancy, confidence
  calibration, and local-first evaluation. Triggers: VibeCheckBench, VibeForge,
  vibecheckbench, vibeforge, fit review, preference fit, setup evaluation.
metadata: {"openclaw":{"requires":{"bins":["node","python3"]}},"codex":{"requires":{"bins":["node","python3"]}}}
---

# VibeForge / VibeCheckBench

**Product:** VibeForge (working brand)  
**Repo / skill id:** `vibecheckbench` (stable until full rename)
...
| Layer | Role |
|---|---|
| User | Natural language / `$vibecheckbench` / `/vibeforge` |
...
node "{baseDir}/scripts/run-fit-eval.mjs" --fit-review vibecheckbench-out --mode mock

# auto: Ollama if available, else mock
node "{baseDir}/scripts/run-fit-eval.mjs" --fit-review vibecheckbench-out --mode auto

# local models
node "{baseDir}/scripts/run-fit-eval.mjs" --fit-review vibecheckbench-out --mode ollama

# hosted — ONLY after user approves API use
node "{baseDir}/scripts/run-fit-eval.mjs" --fit-review vibecheckbench-out --mode openai
node "{baseDir}/scripts/run-fit-eval.mjs" --fit-review vibecheckbench-out --mode anthropic
```
...
```bash
node bin/vibecheckbench.mjs eval --mode mock
node bin/vibecheckbench.mjs eval --mode ollama
```
...
Writes `vibecheckbench-out/` with `VIBE_REPORT.md`, `fit-report.html`,
