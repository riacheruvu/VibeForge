...
function parseArgs(argv) {
  const args = { preference: "", preferenceFile: "", out: "vibecheckbench-out", force: false };
  const positional = [];
...
- Created: ${createdAt}
- Source: local deterministic VibeCheckBench drafter
...
<head>
  ...
  <title>VibeCheckBench Fit Review</title>
  <style>
    :root {
      color-scheme: light;
      --ink: #17223b;
...
<body>
  <main>
    <header>
      <div class="eyebrow">VibeCheckBench fit review</div>
...
export function createFitReview({ preferenceText, outDir = "vibecheckbench-out", force = false }) {
  ...
  const isDefaultOut = path.normalize(outDir) === path.normalize("vibecheckbench-out");
  ...
  const evalCases = {
    version: "vibecheckbench-eval-cases-v1",
    cases: [draft],
  };
  const runResults = {
    version: "vibecheckbench-run-results-v1",
    status: "not_run",
    note: "No model comparison has been run yet. This file is reserved for future baseline/candidate results.",
    results: [],
  };
  const provenance = {
    version: "vibecheckbench-provenance-v1",
    createdAt,
    networkCalls: false,
    source: {
