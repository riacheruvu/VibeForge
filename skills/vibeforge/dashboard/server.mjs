...
const PORT = Number(
  portArgIndex >= 0 ? process.argv[portArgIndex + 1] : process.env.VIBEFORGE_PORT || 4173
);
  ...
  return fs.readdirSync(CASE_STUDIES_DIR, { withFileTypes: true })
    ...
    .map(entry => {
      ...
      return {
        ...
        runCommand: `Ask Codex: Use VibeForge / VibeForge to run the "${payload.title || entry.name}" case study and summarize what changed.`,
      };
    })
    .filter(Boolean)
...
async function executeModelComparison(record, dir) {
  ...
  const payload = { version: "vibeforge-local-results-v1", results: { results: rows } };
...
async function executeConfigGate(record, dir) {
  ...
  record.gate = {
    version: "vibeforge-config-gate-v1",
    development,
    heldout,
    decision: {
