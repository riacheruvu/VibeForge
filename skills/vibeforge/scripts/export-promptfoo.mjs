#!/usr/bin/env node
/**
 * Export a VibeForge preference profile to a Promptfoo regression suite.
 *
 * This keeps VibeForge focused on preference/case authoring and lets
...
function usage() {
  console.log(`VibeForge Promptfoo exporter

Usage:
  node skills/vibeforge/scripts/export-promptfoo.mjs --out promptfooconfig.yaml
...
function buildConfig({ profile, preferences, cases, prompt, providers, threshold }) {
  ...
  return [
    ...
    `description: ${yamlQuote(`VibeForge regression suite: ${profile.name || "profile"}`)}`,
    "",
    "prompts:",
    "  - |",
    yamlBlock(`${prompt}\n\nUser request:\n{{user_prompt}}`, 4),
