"""
VibeForge local runner - llama.cpp backend (llama-cpp-python).

Requires:
    pip install llama-cpp-python

Set VIBEFORGE_GGUF_PATH to the GGUF model file you want to use, e.g.:
    export VIBEFORGE_GGUF_PATH=~/models/phi-3-mini-4k-instruct-q4.gguf

If VIBEFORGE_GGUF_PATH is not set, the runner will exit with a clear error.
"""
...
DEFAULT_TEST_CASES = min(20, max(1, int(os.environ.get("VIBEFORGE_NUM_CASES", "10"))))
DEFAULT_SYSTEM_PROMPT = "You are a helpful assistant."
DEFAULT_LOCAL_FAST = os.environ.get("VIBEFORGE_LOCAL_FAST", "1").strip().lower() not in {"0", "false", "no"}
GGUF_PATH = os.environ.get("VIBEFORGE_GGUF_PATH", "").strip()

CTX_SIZE = int(os.environ.get("VIBEFORGE_CTX_SIZE", "2048"))
N_THREADS = int(os.environ.get("VIBEFORGE_N_THREADS", "4"))
N_GPU_LAYERS = int(os.environ.get("VIBEFORGE_N_GPU_LAYERS", "0"))  # 0 = CPU only
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run VibeForge against a local GGUF model via llama-cpp-python.")
    ...
    parser.add_argument("--model", default=GGUF_PATH, help="Path to GGUF model file (overrides VIBEFORGE_GGUF_PATH)")
...
def run_VibeForge(user_intent, config_b_prompt, model_path, test_cases):
    if not model_path:
        print(
            "No GGUF model path provided.\n"
            "Set VIBEFORGE_GGUF_PATH or pass --model <path/to/model.gguf>",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    report = run_VibeForge(args.intent, prompt, args.model, test_cases)
...
if __name__ == "__main__":
    try:
        ...
        print(f"VibeForge local error: {error}", file=sys.stderr)
        sys.exit(1)
