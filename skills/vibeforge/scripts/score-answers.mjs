...
function usage() {
  ...
  console.log(`Usage:
  node skills/vibeforge/scripts/score-answers.mjs --input answers.json --out reports/results.captured.json
...
export function scoreAnswersFile({ input, out = "reports/results.captured.json", stdout = false, quiet = false }) {
  ...
  const payload = {
    version: "vibeforge-captured-answers-v1",
    metadata: {
  ...
  if (isQuiet()) {
    console.log(`Wrote scored results: ${outPath}`);
      ...
      next: [
        ...
        `Contributor: node skills/vibeforge/scripts/chart-results.mjs --input ${out} --out reports/skill-chart.html`,
      ],
